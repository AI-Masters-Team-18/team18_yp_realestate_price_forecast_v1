...
def main():
    ...
    if uploaded_file is not None:
        ...
        columns_to_drop = [
            "ссылка", "Санузел", "Кол-во санузлов", "описание", "Временная эпоха", "Отопление",
            "Балкон/лоджия", "Серия дома", "Кондиционер", "Кухонная мебель", "Холодильник",
            "Хорошая школа", "Фитнес клуб", "Детский сад", "Посудомойка", "Интернет", "Телевизор",
            "Торговый центр", "Хорошие школы", "Стиральная машина", "Source_File", "Sheet_Name", "адрес",
        ...
        st.write("Мы заметили, что значения в столбце Срок сдачи представляют собой строки, содержащие информацию о квартале и годе, например, '4\xa0кв.\xa02025\xa0г.', или только год, например, '2025\xa0г.'. Также встречаются значения nan и 'Неизвестно'. Однако для анализа нас интересует только год. Поэтому мы извлекли год из строк, заменили 'Неизвестно' на NaN, а некорректные значения преобразовали в 0, после чего перевели данные в целочисленный формат.")
        ...
        df_filtered['Год постройки'] = df_filtered['Год постройки'].where(df_filtered['Год постройки'] > 0,
                                                                          df_filtered['Срок сдачи'])
        ...
        columns_to_convert = [
            'время_до_станции1', 'пешком1',
            'время_до_станции2', 'пешком2',

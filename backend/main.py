...
def get_model_info(mid: str):
    ...
    if pipeline is None:
        return ModelInfo(
            model_id=mid, description=MODELS[mid]["description"], is_active=MODELS[mid]["is_active"], detailed_info=None
        )
...
def get_eda():
    ...
    try:
        ...
        info = {"rows": df.shape[0], "cols": df.shape[1], "columns": list(df.columns), "describe": desc}
...
def get_learning_curve(req: FitRequest):
    ...
    try:
        train_sizes_abs, train_scores, test_scores, _, _ = learning_curve(

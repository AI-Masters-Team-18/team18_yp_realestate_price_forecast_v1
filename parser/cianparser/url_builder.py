...
class URLBuilder:
    ...
    def add_additional_settings(self, additional_settings):
        ...
        # mintarea & maxtarea
        if "maxtarea" in additional_settings.keys():
            self.url += MAX_AREA.format(additional_settings["maxtarea"])
        if "mintarea" in additional_settings.keys():
            self.url += MIN_AREA.format(additional_settings["mintarea"])

        # minfloor & maxfloor
        if "maxfloor" in additional_settings.keys():
            self.url += MAX_FLOOR.format(additional_settings["maxfloor"])
        if "minfloor" in additional_settings.keys():
            self.url += MIN_FLOOR.format(additional_settings["minfloor"])
